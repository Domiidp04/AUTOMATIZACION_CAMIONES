...
function setupEventListeners() {
    ...
    document.getElementById('add').addEventListener('click', function() {
        window.open(ruta + "/truck/html/FormAddVehiculo.html?num=" + codigocliente, '_blank');
    });

    document.getElementById('delete').addEventListener('click', deleteVehicle);
    document.getElementById('loop').addEventListener('click', toggleAutomationPanel);
    ...
    document.getElementById('reset-btn').addEventListener('click', resetAutomation);
}
...
function showselect() {
    ...
    fetch(ruta + "/truck/scr/base.php", {
        ...
        body: `carroceria=${encodeURIComponent(carroceria)}&cliente=${encodeURIComponent(cliente)}`
    })
    ...
    .then(data => {
        try {
            ...
            selectAllCheckbox.addEventListener('change', function() {
                const checkboxes = vehicleList.querySelectorAll('.vehicle-checkbox:not(#select-all)');
                checkboxes.forEach(checkbox => {
                    checkbox.checked = this.checked;
                    if (this.checked && !selectedVehicles.includes(checkbox.value)) {
                        selectedVehicles.push(checkbox.value);
                    } else if (!this.checked && selectedVehicles.includes(checkbox.value)) {
                        selectedVehicles = selectedVehicles.filter(v => v !== checkbox.value);
                    }
                });
                updateSelectionInfo();
            });
            ...
            vehicles.forEach(function(obj) {
                ...
                checkbox.addEventListener('change', function() {
                    ...
                    const allCheckboxes = vehicleList.querySelectorAll('.vehicle-checkbox:not(#select-all)');
                    const checkedBoxes = vehicleList.querySelectorAll('.vehicle-checkbox:not(#select-all):checked');
                    selectAllCheckbox.checked = allCheckboxes.length === checkedBoxes.length && checkedBoxes.length > 0;
...
function deleteVehicle() {
    ...
    if (!confirm(`¿Deseas borrar ${vehicleCount} ${vehicleText} seleccionado(s)?`)) {
        return;
    }
...
function startAutomation() {
    ...
    addLog(`� Iniciando automatización para ${vehicleCount} ${vehicleText}`, 'info');
...
function processNextVehicle() {
    ...
    .then(data => {
        try {
            ...
            chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
                ...
                chrome.scripting.executeScript({
                    ...
                    chrome.tabs.sendMessage(tabs[0].id, {
                        ...
                        setTimeout(() => {
                            sendAutomationMessage(tabs[0].id, vehicleData, codigo);
                        }, 500);
                    });
                });
            });
            ...
            addLog(`❌ Error procesando datos del vehículo ${codigo}: ${error.message}`, 'error');
...
function sendAutomationMessageDirect(tabId, vehicleData, codigo) {
    ...
    chrome.tabs.sendMessage(tabId, messageData, function(response) {
        ...
        if (lastError) {
            ...
            addLog(`❌ Error comunicación con ${codigo}: ${lastError.message}`, 'error');
            ...
            setTimeout(() => {
                ...
                chrome.tabs.sendMessage(tabId, {
                    ...
                    if (retryError) {
                        ...
                        addLog(`❌ Fallo definitivo ${codigo}: ${retryError.message}`, 'error');
                        ...
                        if (retryResponse.success) {
                            addLog(`✅ Comunicación establecida con ${codigo} (reintento)`, 'success');
                            updateStatus(`Procesando ${codigo}`, 'info');
                            toggleButtons(true);
                            showProgress();
                        } else {
                            addLog(`❌ Error en reintento ${codigo}: ${retryResponse.error}`, 'error');
                            setTimeout(() => {
                                currentProcessingIndex++;
                                processNextVehicle();
                            }, 2000);
            ...
            if (response.success) {
                ...
                addLog(`❌ Error del content script ${codigo}: ${response.error}`, 'error');
...
function completeAllAutomation() {
    ...
    addLog(`🎉 ¡Automatización completada! ${processedCount}/${totalCount} vehículos procesados`, 'success');
...
function updateProgress(current, total) {
    if (isProcessingQueue) {
        ...
        const globalProgress = (vehiclesCompleted + currentVehicleProgress) / totalVehicles;
        ...
        if (current < total) {
            updateStatus(`Vehículo ${currentProcessingIndex + 1}/${totalVehicles} - Paso ${current}/${total}`, 'running');
        }
    } else {
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
    switch (message.type) {
        case 'STATUS_UPDATE':
            updateStatus(message.data.text, message.data.type);
            break;
        case 'PROGRESS_UPDATE':
            if (message.data && message.data.current !== undefined && message.data.total !== undefined) {
        ...
        case 'AUTOMATION_COMPLETE':
            if (isProcessingQueue) {
                // Un vehículo completado, pasar al siguiente
                const completedVehicle = vehicleQueue[currentProcessingIndex];
                addLog(`✅ Vehículo ${completedVehicle} completado (${currentProcessingIndex + 1}/${vehicleQueue.length})`, 'success');
                
                currentProcessingIndex++;
                
                // Pausa reducida para procesar más rápido
                setTimeout(() => {
                    processNextVehicle();
                }, 2000); // Reducido de 3000 a 2000ms
            } else {
                // Automatización individual completada
